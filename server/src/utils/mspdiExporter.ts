 ...
 * Returns empty string if date is invalid (MS Project will handle empty dates)
 */
function formatMspdiDate(dateStr: string | null | undefined): string {
    if (!dateStr) return '';
    try {
        ...
        // MSPDI uses ISO 8601 format: YYYY-MM-DDTHH:MM:SS
...
<Project xmlns="http://schemas.microsoft.com/project">
    ...
    <StartDate>${formatMspdiDate(projectStartDate)}</StartDate>
    <FinishDate>${formatMspdiDate(projectFinishDate)}</FinishDate>
...
`;
    ...
    xml += `        <Task>
            ...
            <Start>${formatMspdiDate(projectStartDate)}</Start>
            <Finish>${formatMspdiDate(projectFinishDate)}</Finish>
            ...
            <EarlyStart>${formatMspdiDate(projectStartDate)}</EarlyStart>
            <EarlyFinish>${formatMspdiDate(projectFinishDate)}</EarlyFinish>
            <LateStart>${formatMspdiDate(projectStartDate)}</LateStart>
            <LateFinish>${formatMspdiDate(projectFinishDate)}</LateFinish>
            ...
            <ActualStart>${formatMspdiDate(projectStartDate)}</ActualStart>
...
`;
    ...
    tasks.forEach((task, index) => {
        ...
        xml += `        <Task>
            ...
            <Priority>500</Priority>
            <Start>${formatMspdiDate(task.startDate)}</Start>
            <Finish>${formatMspdiDate(task.finishDate)}</Finish>
            <Duration>${formatMspdiDuration(task.duration)}</Duration>
            <DurationFormat>7</DurationFormat>
            <Work>${formatMspdiWork(task.effort)}</Work>
            <ResumeValid>0</ResumeValid>
            ...
            <IsSubprojectReadOnly>0</IsSubprojectReadOnly>
            <ExternalTask>0</ExternalTask>
            <EarlyStart>${formatMspdiDate(task.startDate)}</EarlyStart>
            <EarlyFinish>${formatMspdiDate(task.finishDate)}</EarlyFinish>
            <LateStart>${formatMspdiDate(task.startDate)}</LateStart>
            <LateFinish>${formatMspdiDate(task.finishDate)}</LateFinish>
            <StartVariance>0</StartVariance>
            <FinishVariance>0</FinishVariance>
            <WorkVariance>0</WorkVariance>
            <FreeSlack>0</FreeSlack>
            ...
            <ActualWork>PT0H0M0S</ActualWork>
            <ActualOvertimeWork>PT0H0M0S</ActualOvertimeWork>
            <RegularWork>${formatMspdiWork(task.effort)}</RegularWork>
            <RemainingDuration>${formatMspdiDuration(task.duration)}</RemainingDuration>
            <RemainingCost>0</RemainingCost>
            <RemainingWork>${formatMspdiWork(task.effort)}</RemainingWork>
            <RemainingOvertimeCost>0</RemainingOvertimeCost>
            <RemainingOvertimeWork>PT0H0M0S</RemainingOvertimeWork>
            <ACWP>0</ACWP>
            <CV>0</CV>
...
`;
    ...
    assignments.forEach(assignment => {
        ...
        xml += `        <Assignment>
            ...
            <CV>0</CV>
            <Delay>0</Delay>
            <Finish>${formatMspdiDate(new Date().toISOString())}</Finish>
            <FinishVariance>0</FinishVariance>
            <WorkVariance>0</WorkVariance>
            ...
            <Start>${formatMspdiDate(new Date().toISOString())}</Start>
            <StartVariance>0</StartVariance>
            <SV>0</SV>
            <Work>PT0H0M0S</Work>
...
export function convertToMspdiFormat(
    ...
    const extractFinishDate = (task: any): string | undefined => {
        // Try Bryntum format first (endDate or finishDate)
        if (task.endDate && typeof task.endDate === 'string') {
            // Bryntum uses exclusive end date, convert to inclusive finish date
            try {
        ...
        // Try Dataverse format (eppm_finishdate)
    ...
    const flattenTasks = (taskList: any[], parentId?: string) => {
        taskList.forEach(task => {
            ...
            flatTasks.push({
                ...
                duration: task.duration || task.eppm_taskduration,
