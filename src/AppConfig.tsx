...
export const useGanttProps = (_handleEditClick: Function): BryntumGanttProps => {
  ...
  return {
    ...
    ...({
      features: {
        ...
        // Task Editor: Advanced tab is enabled by default in Bryntum Gantt
        // All advanced fields (calendar, schedulingMode, constraintType, etc.) are supported natively
        // The fields will appear automatically when present in task data
        taskEdit: {
    ...
    tbar: [
...

      {
        ...
        onAction: async () => {
          try {
            ...
            const response = await fetch(`${API_URL}/tasks/export/mpp?projectName=Gantt_Export`, {
              method: "GET",
              headers: {
            ...
            let filename = `Gantt_Export_${new Date().toISOString().split("T")[0]}.xml`;
      ...
      },
      {
        ...
        onAction: async ({ source }: any) => {
          ...
          try {
            console.log("[Import] Starting MS Project import...");
            ...
            fileInput.onchange = async (event: Event) => {
              ...
              try {
                ...
                const response = await fetch(`${API_URL}/tasks/import/mpp`, {
                  method: "POST",
                  headers: {
                    Authorization: `Bearer ${token}`,
                  },
      ...
      transport: {
        load: {
          ...
          fetch: async (url: string, options: any = {}) => {
            ...
            // Bryntum might pass a relative URL, so we ensure it's absolute
            let requestUrl = url;
            if (
              !requestUrl ||
              requestUrl === "/" ||
              ...
              requestUrl = `${API_URL}/tasks`;
              console.log(
                "[Bryntum Load] URL was invalid/relative, using:",
                requestUrl
              );
            }
            ...
            if (options) {
              ...
              // Copy other options but don't override headers
            ...
            console.log("[Bryntum Load] Response headers:", Object.fromEntries(response.headers.entries()));
    ...
    columns: [
      ...
      { type: "startdate", text: "Start Date", width: 120 },
      {
        ...
        text: "End Date",
        width: 120,
        // Bryntum stores end date as exclusive (day after last day). Display inclusive finish date (last day) so e.g. 5 days from Feb 4 shows "Feb 10".
        // renderer: ({ value }: any) => {
      ...
      { type: "duration", text: "Duration", width: 120 },
